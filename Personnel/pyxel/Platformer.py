...
class Game:
    ...
    def isallowedmove(self, move):
        ...
        while abs(addy) <= abs(move[1]) and move[1] != 0:
            if move[1] > 0:
                y1 = px.pget(self.player.x+3, self.player.y+addy+8)
                y1 = y1 in [1, 3, 5, 11]
                y2 = px.pget(self.player.x+5, self.player.y+addy+8)
                y2 = y2 in [1, 3, 5, 11]
            else:
                y1 = px.pget(self.player.x, self.player.y+addy-1)
                y1 = y1 in [1, 3, 5, 6, 7, 11]
                y2 = px.pget(self.player.x+7, self.player.y+addy-1)
                y2 = y2 in [1, 3, 5, 6, 7, 11]
            if y1 and y2:
                if move[1] < 0:
                    addy -= 0.5
                elif move[1] > 0:
